...
define("tinymce/pasteplugin/SmartPaste", [
	...
	function uploadImages(editor,text) {
		...
		for (var i = 0; i < images.length; i++) {
			...
			wikiImageObject = {};
/* DC save for later if we need it
			//process if link
...
*/
			...
			for (var j = 0; j < attributes.length; j++) {
				...
				if (property == 'src' ) {
					var srcName = attributes[j].value;
				} 
			}
...
//		editor.insertContent(html, {
	...
	var smartInsertContent = function (editor, html) {
		// upload any included images, if upload fails replace image tag with a message
		html = uploadImages(editor,html);
		Tools.each([
			linkSelection,
			insertImage,
			pasteHtml
...
define("tinymce/pasteplugin/Clipboard", [
	...
	return function(editor) {
		...
		/**
		 * Creates a paste bin element as close as possible to the current caret location 
		 * and places the focus inside that element
				...
				if (items) {
					for (i = 0; i < items.length; i++) {
						...
						if (/^image\/(jpeg|png|gif|bmp)$/.test(item.type)) {
							editor.windowManager.alert("Loading local image files when they are pasted or dragged into the editor window is not supported.  Try uploading the file using the menu button instead" );
/*  DC removed the following as if we cannot drop and uipload to the wiki we should get the user to upload manually		

							var blob = item.getAsFile ? item.getAsFile() : item;
							var path = blob.webkitGetAsEntry();
...
define("tinymce/pasteplugin/Plugin", [
	...
	PluginManager.add('wikipaste', function(editor) {
